package main

import (
	"fmt"
	"math"
)

func float64FromInt16(v int16) (abs float64, sign float64) {
	if v < 0 {
		return float64(-int(v)) / 0x8000, -1
	} else {
		return float64(v) / 0x7fff, 1
	}
	return 0, 0 // not reached
}

type viewaccumulatelogic struct {
	x, y float64
	s    float64
}

func (v *viewaccumulatelogic) update(c *Config, xi, yi float32) (xo, yo float32) {
	if c.HeadLook != nil {
		xv, yv := float64(xi), float64(yi)
		if tiny(xv) && tiny(yv) {
			v.centeraccel(c.HeadLook.acapertick, c.HeadLook.AutoCenterDist)
		} else {
			v.x += xv * c.HeadLook.movepertick
			v.y += yv * c.HeadLook.movepertick
			if v.x < -1 {
				v.x = -1
			}
			if 1 < v.x {
				v.x = 1
			}
			if v.y < -1 {
				v.y = -1
			}
			if 1 < v.y {
				v.y = 1
			}
		}
	}
	return float32(v.x), float32(v.y)
}

func (v *viewaccumulatelogic) jumpToOrigin(c *Config) (xo, yo float32) {
	if c.HeadLook != nil {
		v.centeraccel(c.HeadLook.jumppertick, 1e6)
	}
	return float32(v.x), float32(v.y)
}

func (v *viewaccumulatelogic) centeraccel(a, limit float64) {
	// d=a/2*t²
	d := math.Sqrt(v.x*v.x + v.y*v.y)
	switch {
	case d < 1e-6:
		v.x, v.y, v.s = 0, 0, 0
	case d < limit:
		t := math.Sqrt(2 * d / a)
		maxs := a * t
		v.s += a
		if v.s > maxs {
			v.s = maxs
		}
		m := 1 - v.s/d
		if m < 0 {
			m = 0
		}
		v.x *= m
		v.y *= m
	}
}

func tiny(v float64) bool {
	return math.Abs(v) < 1e-3
}

func viewauto(pv *float64, limit, move float64) {
	v := *pv
	var vabs, vsign float64
	switch {
	case v <= -limit:
		vabs, vsign = -v, -1
	case v >= limit:
		vabs, vsign = v, 1
	default:
		*pv = 0
		return
	}
	if vabs <= limit {
		if move < vabs {
			*pv = v - vsign*move
		} else {
			*pv = 0
		}
	}
}

func triggermap(v, thr uint16, pow float64) float32 {
	if v <= thr {
		return 0
	}
	vv := float64(v-thr) / float64(255-thr)
	return float32(math.Pow(vv, pow))
}

type StickPos struct {
	X, Y float64
}

func (p *StickPos) Init(x, y int16) {
	p.X = float64sFromInt16(x)
	p.Y = float64sFromInt16(y)
}

func (p *StickPos) Apply(l StickLogic) {
	l.Visit(p)
}

type StickLogic interface {
	Visit(*StickPos)
}

type StickFunc func(p *StickPos)

func (f StickFunc) Visit(p *StickPos) {
	f(p)
}

type AxisFunc func(float64) float64

func XAxisLogic(a AxisFunc) StickLogic {
	return StickFunc(func(p *StickPos) {
		p.X = a(p.X)
	})
}

func YAxisLogic(a AxisFunc) StickLogic {
	return StickFunc(func(p *StickPos) {
		p.Y = a(p.Y)
	})
}

func XYAxisLogic(xa, ya AxisFunc) StickLogic {
	return StickFunc(func(p *StickPos) {
		p.X = xa(p.X)
		p.Y = ya(p.Y)
	})
}

func float64sFromInt16(v int16) float64 {
	if v < 0 {
		return float64(v) / 0x8000
	} else {
		return float64(v) / 0x7fff
	}
	return 0 // not reached
}

type ErrArguments struct {
	Name string
	Argv []interface{}
}

func (e *ErrArguments) Error() string {
	return fmt.Sprint("Argument error: ", e.Name, e.Argv)
}

func NewStickLogic(c *Config, name string, args []interface{}) (l StickLogic, err error) {
	defer func() {
		if r := recover(); r != nil {
			l, err = nil, &ErrArguments{name, args}
		}
	}()
	return newStickLogic(c, name, args), nil
}

func newStickLogic(c *Config, name string, args []interface{}) StickLogic {
	switch name {
	case "circularize":
		if len(args) <= 1 {
			factor := float64(1)
			if len(args) == 1 {
				factor = args[0].(float64)
			}
			of, mf := 1-factor, factor
			return StickFunc(func(p *StickPos) {
				if p.X*p.X+p.Y*p.Y < 1e-3 {
					return
				}

				xa, ya := math.Abs(p.X), math.Abs(p.Y)

				var u float64
				if xa > ya {
					u = ya / xa // yu/xu = ya/xa
				} else {
					u = xa / ya // xu/yu = xa/ya
				}
				m := math.Sqrt(1+u*u) * mf
				p.X *= (of + m)
				p.Y *= (of + m)
			})
		}
	case "xy", "x", "y":
		aname := args[0].(string)
		switch name {
		case "x":
			return XAxisLogic(newAxisFunc(c, aname, args[1:]))
		case "y":
			return YAxisLogic(newAxisFunc(c, aname, args[1:]))
		case "xy":
			return XYAxisLogic(newAxisFunc(c, aname, args[1:]), newAxisFunc(c, aname, args[1:]))
		}
	default:
		return XYAxisLogic(newAxisFunc(c, name, args), newAxisFunc(c, name, args))
	}
	panic("errarg")
}

func newAxisFunc(c *Config, name string, args []interface{}) (a AxisFunc) {
	switch name {
	case "deadzone":
		if len(args) == 1 {
			if dz, ok := args[0].(float64); ok && 0 <= dz && dz < 1 {
				dzmult := 1 / (1 - dz)
				return func(v float64) float64 {
					var s float64
					if v < 0 {
						v, s = -v, -1
					} else {
						s = 1
					}
					if v < dz {
						return 0
					}
					return v * dzmult * s
				}
			}
		}
	case "multiplier":
		if len(args) == 1 {
			m := args[0].(float64)
			return func(v float64) float64 {
				return v * m
			}
		}
	case "curvature":
		if len(args) == 1 {
			p := math.Pow(2, args[0].(float64))
			return func(v float64) float64 {
				s := float64(1)
				if v < 0 {
					s, v = -1, -v
				}
				return s * math.Pow(v, p)
			}
		}
	case "truncate":
		if len(args) <= 1 {
			t := float64(1)
			if len(args) == 1 {
				t = args[0].(float64)
			}
			return func(v float64) float64 {
				switch {
				case v < -t:
					return -t
				case t < v:
					return t
				}
				return v
			}
		}
	case "lag":
		if len(args) == 2 {
			dt := float64(c.UpdateMicros) / 1e6
			speed := args[0].(float64) * dt
			rebound := args[1].(float64) * dt
			var pos float64
			return func(v float64) float64 {
				if math.Abs(v) < 1e-3 {
					switch {
					case pos < -rebound:
						pos += rebound
					case rebound < pos:
						pos -= rebound
					default:
						pos = 0
					}
				} else {
					if 0 <= pos*v {
						pos += v * speed
						switch {
						case pos < -1:
							pos = -1
						case 1 < pos:
							pos = 1
						}
					} else {
						pos = 0
					}
				}
				return pos
			}
		}
	}
	panic("errargs")
}
